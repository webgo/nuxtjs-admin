  ...
  <div class="font-sans bg-white text-black w-full min-h-screen">
    ...
    <div class="flex items-center justify-center py-16 px-5">
      <div class="w-full max-w-[420px]">
        <h1 class="text-[28px] font-bold mb-2">{{ $t('auth.loginTitle') }}</h1>
        ...
        <p class="text-center text-sm text-gray-500 mt-6">
          {{ $t('auth.noAccount') }}
          <NuxtLink to="/portal/register" class="text-black font-semibold underline ml-1">
            {{ $t('auth.goRegister') }}
          </NuxtLink>
        </p>
      </div>
...
async function handleLogin() {
  ...
  loading.value = true
  try {
    await portalAuth.login(form.email, form.password)
    navigateTo('/portal')
  } catch (err: unknown) {
    const error = err as { data?: { message?: string }; message?: string }
    errorMsg.value = error.data?.message || error.message || t('auth.loginFailed')
  } finally {
    loading.value = false
  }
}
