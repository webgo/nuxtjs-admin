<template>
  <div class="role-page">
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="{ row }">
            <template v-if="row.code === 'admin'">
              <el-tag size="small">内置角色</el-tag>
            </template>
            <template v-else>
              <el-button type="primary" link size="small" @click="handleEdit(row as unknown as RoleItem)">修改</el-button>
              <el-button type="success" link size="small" @click="handlePermission(row as unknown as RoleItem)">权限</el-button>
              <el-button type="danger" link size="small" @click="handleDelete(row as unknown as RoleItem)">删除</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 角色编辑对话框 -->
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑角色' : '新增角色'" width="550px">
...
function handleAdd() {
  ...
  form.sort = 0; form.status = 1; form.remark = ''
